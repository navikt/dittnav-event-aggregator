...
class OppgaveTransformerTest {
    ...
    fun `should transform external to internal`() {
        val external = OppgaveObjectMother.createOppgave(1)

        val internal = OppgaveTransformer.toInternal(external)

        internal.aktorId `should be equal to` external.getAktorId()
        internal.dokumentId `should be equal to` external.getDokumentId()
